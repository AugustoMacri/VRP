...
double array_fitness_distance[30];
double array_fitness_time[30];
double array_fitness_fuel[30];
double array_fitness[30];
int aux = 0;
...
int main()
{
    ...
    for (int i = 0; i < ROUNDS; i++)
    {
        ...
        if ((i % 100) == 0)
        {
            ...
            if (aux < 30)
            {
                array_fitness_distance[aux] = subPopDistance[0].fitnessDistance;
                array_fitness_time[aux] = subPopTime[0].fitnessTime;
                array_fitness_fuel[aux] = subPopFuel[0].fitnessFuel;
                array_fitness[aux] = subPopWeighting[0].fitness;
                aux++;
            }
            //printf("Rodada %d\n", i);
        }
        ...
        if (i == 0)
        {
            ...
            printf("first - distance: %.4f | time: %.4f | fuel: %.4f | pond: %.4f\n", firstFitDist, firstFitTime, firstFitFuel, firstFitWeight);
        }
    }
    ...
    else
    {
        ...
        // imprimir o fitness do array
        for (int i = 0; i < 30; i++)
        {
            fprintf(file, "%.2f", array_fitness_distance[i]);
            if (i < 29)
                fprintf(file, ", ");
        }
        fprintf(file, "\n");
        ...
        for (int i = 0; i < 30; i++)
        {
            fprintf(file, "%.2f", array_fitness_time[i]);
            if (i < 29)
                fprintf(file, ", ");
        }
        fprintf(file, "\n");
        ...
        for (int i = 0; i < 30; i++)
        {
            fprintf(file, "%.2f", array_fitness_fuel[i]);
            if (i < 29)
                fprintf(file, ", ");
        }
        fprintf(file, "\n");
        ...
        for (int i = 0; i < 30; i++)
        {
            fprintf(file, "%.2f", array_fitness[i]);
            if (i < 29)
                fprintf(file, ", ");
        }
        fprintf(file, "\n");
